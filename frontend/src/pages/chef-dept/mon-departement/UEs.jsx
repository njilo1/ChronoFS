...
export default function ChefUEs() {
  const { data, loading, create, update, remove } = useCrud('mon-departement/ues', { nom: 'UE', genre: 'f' });
  ...
  return (
    <PageShell
      icon={BookOpen}
      title="Mes UEs"
      subtitle="Unités d'enseignement de votre département"
      count={loading ? null : data.length}
      action={
        <Button onClick={openCreate} disabled={filieres.length === 0}>
    ...
    >
      ...
      <Table columns={COLS} data={filtered} loading={loading}
        ...
        emptyText="Aucune UE. Cliquez sur « Ajouter » pour créer la première." />
